...
def entonces_obtengo_el_area_de_circulo_group1(step, esperado):
    assert float(esperado) == world.areacir,'El resultado esperado de '+esperado+" y el obtenido es "+str(world.areacir)
...
def entonces_obtengo_el_area_de_cuadrado_group1(step, esperado):
    assert int(esperado) == world.areacuad,'El resultado esperado de '+esperado+" y el obtenido es "+str(world.areacuad)
...
def entonces_obtengo_el_area_de_rectangulo_group1(step, esperado):
    assert int(esperado) == world.arearec,'El resultado esperado de '+esperado+" y el obtenido es "+str(world.arearec)
@step(u'Dado que tengo la base mayor "([^"]*)", base menor "([^"]*)" y altura "([^"]*)"')
def dado_que_tengo_la_base_mayor_group1_base_menor_group2_y_altura_group2(step, basemay, basemen, altura):
    world.area = AreaFiguras()
    world.areatrap = world.area.obtenAreaTrapecio(int(basemay), int(basemen), int(altura))
@step(u'entonces obtengo el area de trapecio "([^"]*)"')
def entonces_obtengo_el_area_de_trapecio_group1(step, esperado):
    assert int(esperado) == world.areatrap,'El resultado esperado de '+esperado+" y el obtenido es "+str(world.areatrap)
...
def entonces_obtengo_el_area_de_triangulo_group1(step, esperado):
    assert int(esperado) == world.areatri,'El resultado esperado de '+esperado+" y el obtenido es "+str(world.areatri)